...
router.get('/:categoryId', async (req, res) => {
    ...
    let getWebtoonListQuery = 'SELECT name, writer, thumbnail FROM webtoon WHERE category = ?'
    ...
    if(result.length === undefined) {
        ...
        res.status(200).send(authUtil.successFalse(statusCode.BAD_REQUEST, resMessage.NO_CATEGORY, []))
